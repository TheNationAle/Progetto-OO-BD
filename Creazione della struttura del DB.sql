...
CREATE OR REPLACE FUNCTION inserisciPostif() RETURNS TRIGGER AS $destinazioneTrigger$
DECLARE
	auto INTEGER;
	persone INTEGER;
BEGIN
	SELECT capienzaa, capienzap INTO auto, persone FROM natante WHERE id_natante = NEW.id_natante;
		
		UPDATE corsa
		SET postirimasti_a = auto, postirimasti_p = persone
		WHERE id_corsa = NEW.id_corsa;
RETURN NEW;
END;
$destinazioneTrigger$ LANGUAGE plpgsql;
